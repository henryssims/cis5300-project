...
Summary:
This project trains and evaluates two T5-based simplification models and several “extension” settings, then compares quality (SARI) and latency across incremental completion ratios (25%, 50%, 75%, 100%). All training is done in Colab using an A100 GPU.

Colab note:
When running any notebook in Colab, upload the `data/` folder from this repo (it contains `train.csv`, `dev.csv`, and `test.csv`), as well as any other relevant files.
...
1) Train the baseline model (writes `t5-simplification/`) / try the extension 1 model (adaptive length penalty)

- Colab: upload and run `code/extension1.ipynb` all the way through. It trains the baseline model and includes cells to zip/download the resulting folder. It will output the SARI score results to the console and also save the model to `t5-simplification/` while comparing the results of the baseline (no adaptive length penalty) and extension 1 (adaptive length penalty).
...
- Colab: upload and run `code/comparison.ipynb` all the way through. It will output the SARI score results to the console and also save the model to `t5-simplification-2/` while comparing the results of the baseline and extension 3 (encoder adapter) against extension 1 (adaptive length penalty) and extension 2 (adaptive decoding). You can also just plug in the weights we got from the previous steps and run the notebook to get the plots. Note that this must be run on Mac M3 chip for the latency results to be consistent with ours. It will output the plots to the cell output.

If you want to run with the latest weights, you can download the zip files from the following Google Drive links:
